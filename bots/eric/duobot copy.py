...
class BotPlayer:
    ...
    def _get_nearest_with_submit_priority(self, pos: Tuple[int, int], 
                                           locations: List[Tuple[int, int]]) -> Optional[Tuple[int, int]]:
        """Get location that minimizes total distance: bot -> location -> submit.
        
        This ensures we pick utilities that are convenient for the full workflow,
        not just close to the bot's current position.
        """
        if not locations:
            return None
        if not self.submits:
        ...
        # Find nearest submit
        nearest_submit = min(self.submits, key=lambda s: self._chebyshev_dist(pos, s))
        
        # Score each location by: distance_from_bot + distance_to_submit
        def score(loc):
            dist_to_loc = self._chebyshev_dist(pos, loc)
            dist_to_submit = self._chebyshev_dist(loc, nearest_submit)
            return dist_to_loc + dist_to_submit
    ...
    def _get_free_counter(self, controller: RobotController, team: Team, 
        ...
        """Find best free counter - balancing distance to bot AND distance to submit."""
        free = []
        for c in self.counters:
            ...
            if tile and getattr(tile, 'item', None) is None:
                free.append(c)
        # Use submit-aware selection for better workflow efficiency
        return self._get_nearest_with_submit_priority(pos, free)
    
    def _get_available_cooker(self, controller: RobotController, team: Team,
                               bot_id: int, pos: Tuple[int, int]) -> Optional[Tuple[int, int]]:
        """Find best cooker with empty pan - balancing distance to bot AND distance to submit."""
        available = []
        for c in self.cookers:
            ...
            if tile:
                ...
                if isinstance(pan, Pan) and pan.food is None:
                    available.append(c)
        # Use submit-aware selection for better workflow efficiency
        return self._get_nearest_with_submit_priority(pos, available)
    ...
    def _estimate_completion_turns(self, controller: RobotController, bot_id: int, 
        ...
        # Dynamic AVG_MOVE based on map size
        # Be realistic - on large maps, utilities can be far apart
        game_map = controller.get_map(team)
        map_diag = max(game_map.width, game_map.height)
        # Use map_diag // 4 for realistic movement times
        # 15x15 map → AVG_MOVE = 3-4, which is realistic for scattered utilities
        AVG_MOVE = max(2, map_diag // 4)
    ...
    def _can_complete_order(self, controller: RobotController, bot_id: int,
        ...
        """Check if bot can realistically complete order before it expires AND before game ends."""
        current_turn = controller.get_turn()
        expires = order.get('expires_turn', 0)
        starts_at = order.get('created_turn', 0)
        
        TOTAL_TURNS = 250  # Game ends at turn 250
        
        # Order hasn't started yet - count from when it starts
        effective_start = max(current_turn, starts_at)
        
        # Time available is the MINIMUM of: order expiry OR game end
        time_until_expiry = expires - effective_start
        time_until_game_end = TOTAL_TURNS - effective_start
        time_available = min(time_until_expiry, time_until_game_end)
        
        # If game ends before order even starts, can't complete
        if time_available <= 0:
            return False
        
        estimated_time = self._estimate_completion_turns(controller, bot_id, order, team)
        
        # Check if estimate fits within available time
        return estimated_time <= time_available
    
    def _get_ingredient_cost(self, order: Dict) -> int:
        """Calculate total cost of ingredients for an order."""
        total_cost = 0
        for ing in order.get('required', []):
            info = INGREDIENT_INFO.get(ing, {})
    ...
    def _get_order_profitability(self, order: Dict) -> float:
        """Calculate actual profit: reward - ingredient_cost.
        
        Returns the net profit if we complete the order.
        Negative means we lose money by completing it.
        """
        reward = order.get('reward', 0)
        ingredient_cost = self._get_ingredient_cost(order)
        return reward - ingredient_cost
    
    def _should_skip_unprofitable(self, order: Dict) -> bool:
        """Check if we should skip an order because completing it loses more money than the penalty.
        
        If profit < -penalty, it's better to skip and pay the penalty.
        Example: MEAT costs $80, reward=$30, penalty=$10
          - Completing: profit = 30 - 80 = -$50 (lose $50)
          - Skipping: penalty = -$10 (lose $10)
          - Better to skip!
        """
        profit = self._get_order_profitability(order)
        penalty = order.get('penalty', 0)
        
        # If completing costs more than the penalty, skip
        # profit < -penalty means |profit| > penalty when profit is negative
    ...
    def _select_order_for_bot(self, controller: RobotController, bot_id: int, team: Team) -> Optional[Dict]:
        ...
        for order in orders:
            ...
            # PROFITABILITY CHECK - skip if completing costs more than penalty
            if self._should_skip_unprofitable(order):
                profit = self._get_order_profitability(order)
                penalty = order.get('penalty', 0)
                self._log(f"    Bot {bot_id}: Skipping order #{order_id} - unprofitable (profit={profit}, penalty={penalty})")
                continue
            
            is_active = starts_at <= current_turn
            
            # FEASIBILITY CHECK
            # For active orders: must complete before expiry AND game end
            # For future orders: can start preparing now, so check if we can 
            #   complete before expiry AND game end, starting from NOW
            TOTAL_TURNS = 250
            estimated_time = self._estimate_completion_turns(controller, bot_id, order, team)
            ...
            if is_active:
                ...
                # Future order: we can start preparing NOW
                # Completion turn is current_turn + estimated_time
                # But we can't submit until order starts, so effective completion is:
                # max(current_turn + estimated_time, starts_at + 1)
                # Add 10% buffer for safety
                buffered_time = int(estimated_time * 1.1)
                completion_turn = max(current_turn + buffered_time, starts_at + 1)
                
                # Must complete BEFORE order expires AND game ends (not at exactly the deadline)
                deadline = min(expires, TOTAL_TURNS)
                self._log(f"    Bot {bot_id}: Order #{order_id} future check: est={estimated_time}, buf={buffered_time}, comp={completion_turn}, deadline={deadline}")
                if completion_turn >= deadline:
                    if completion_turn >= TOTAL_TURNS:
                        self._log(f"    Bot {bot_id}: Skipping order #{order_id} - would complete at/after game ends (turn {completion_turn})")
                    else:
                        self._log(f"    Bot {bot_id}: Skipping order #{order_id} - would complete at/after expiry (turn {completion_turn} vs {deadline})")
                    continue
